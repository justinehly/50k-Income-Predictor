...
pc.result<-prcomp(clean.pca,scale.=FALSE)
...
# Scree plot shows we really only need 2 continuous variables.
...
unbal <- accuracy.meas(test$Income, pred.treeimb[,2])
unbal 

# threshold = 0.5
# not very good numbers 
# precision = 0.719 {precision of prediction}
# recall = 0.460 {recall of the prediction}
# F = 0.280 {average between precision and recall}
...
# build decision tree to evaluate accuracy
...
precision <- as.numeric(unlist(c(unbal[3], ptu[3])))
recall <- as.numeric(unlist(c(unbal[4], ptu[4])))
F <- as.numeric(unlist(F <- c(unbal[5], ptu[5])))

tree <- c("Unbalanced",  "Under Sampled")
# put all the accuract measures into 1 dataframe
accuracyDF <- data.frame(tree, precision, recall, F)
accuracyDF
# We end up with reversed precision and recall, let's avoid this for now.
...
dat.train.x <- model.matrix(Income~.,train)
dat.train.y<-train[,1]
...
mod.lasso <- model.matrix(Income~age + workclass + fnlwgt + education +
