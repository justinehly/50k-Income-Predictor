...
chisq.test(train$native.country, train$age)
...
train %>% ggplot(aes(x = native.country, y = age, color = Income)) +geom_boxplot()+
  theme_classic() +
  theme(axis.text.x = element_text(angle = 270, vjust = 0, hjust = 0))
...
# Table for Accuracy, Sensitivity, Specificity
cutoff.lda.qda <- c(cutoff.lasso, cutoff.step, cutoff.lasso.complex, cutoff.lda, cutoff.lda.pca, cutoff.qda.pca)
sens.lda.qda <- c(sens.lasso, sens.step, sens.lasso.complex, sens.lda, sens.lda.pca, sens.qda.pca)
spec.lda.qda <- c(spec.lasso, spec.step, spec.lasso.complex, spec.lda, spec.lda.pca, spec.qda.pca)
acc.lda.qda <- c(acc.lasso, acc.step, acc.lasso.complex, acc.lda, acc.lda.pca, acc.qda.pca)
auc.lda.qda <- c(auc.lasso@y.values[[1]], auc.step@y.values[[1]], auc.lasso.complex@y.values[[1]], NA, NA, NA)
